...
registerSuite('Calendar DatePicker', {
	tests: {
		...
		'Month popup closes with correct Key'() {
			let isOpen = true;
			const h = harness(() => w(DatePicker, {
				onPopupChange: (open: boolean) => { isOpen = open; },
				...requiredProps
			...
			h.trigger('@month-button', 'onclick', stubEvent);
			h.expect(() => expected(true, false));
			h.trigger(`.${css.monthGrid} fieldset`, 'onkeydown', { which: Key.Escape, ...stubEvent });
			h.expect(() => expected(false, false));
			...
			h.trigger('@month-button', 'onclick', stubEvent);
			h.expect(() => expected(true, false));
			h.trigger(`.${css.monthGrid} fieldset`, 'onkeydown', { which: Key.Enter, ...stubEvent });
			h.expect(() => expected(false, false));
			...
			h.trigger('@month-button', 'onclick', stubEvent);
			h.expect(() => expected(true, false));
			h.trigger(`.${css.monthGrid} fieldset`, 'onkeydown', { which: Key.Space, ...stubEvent });
			h.expect(() => expected(false, false));
			...
			h.trigger('@month-button', 'onclick', stubEvent);
			h.expect(() => expected(true, false));
			h.trigger(`.${css.monthGrid} fieldset`, 'onkeydown', { which: Key.PageDown, ...stubEvent });
			h.expect(() => expected(true, false));
			assert.isTrue(isOpen, 'Other Key don\'t close popup');
		},

		'year popup closes with correct Key'() {
			let isOpen = true;
			const h = harness(() => w(DatePicker, {
				onPopupChange: (open: boolean) => { isOpen = open; },
				...requiredProps
			...
			h.trigger('@year-button', 'onclick', stubEvent);
			h.expect(() => expected(false, true));
			h.trigger(`.${css.yearGrid} fieldset`, 'onkeydown', { which: Key.Escape, ...stubEvent });
			h.expect(() => expected(false, false));
			assert.isFalse(isOpen, 'Should close on escape key press');
			...
			h.trigger('@year-button', 'onclick', stubEvent);
			h.expect(() => expected(false, true));
			h.trigger(`.${css.yearGrid} fieldset`, 'onkeydown', { which: Key.Enter, ...stubEvent });
			h.expect(() => expected(false, false));
			assert.isFalse(isOpen, 'Should close on enter key press');
			...
			h.trigger('@year-button', 'onclick', stubEvent);
			h.expect(() => expected(false, true));
			h.trigger(`.${css.yearGrid} fieldset`, 'onkeydown', { which: Key.Space, ...stubEvent });
			h.expect(() => expected(false, false));
			assert.isFalse(isOpen, 'Should close on space key press');
			...
			h.trigger('@year-button', 'onclick', stubEvent);
			h.expect(() => expected(false, true));
			h.trigger(`.${css.yearGrid} fieldset`, 'onkeydown', { which: Key.PageDown, ...stubEvent });
			h.expect(() => expected(false, true));
			assert.isTrue(isOpen, 'Other Key don\'t close popup');
		},
