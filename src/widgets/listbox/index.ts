 ...
 * @property multiple             Adds currect semantics for a multiselect listbox
...
export interface ListboxProperties extends RedaktorProperties, CustomAriaProperties {
	activeIndex?: number;
...
export class ListboxBase<P extends ListboxProperties = ListboxProperties> extends RedaktorWidgetBase<P> {
	...
	protected getModifierClasses() {
		const { raised, visualFocus } = this.properties;
  ...
  protected _getOptionLabel(option: any) {
	...
	protected renderOptionLabel(option: any, index: number): DNode {
		...
		return getOptionLabel ? getOptionLabel(option, index) : this._getOptionLabel(option)
	}

	protected renderOption(option: any, index: number): DNode {
		const {
      widgetId = this._idBase, activeIndex = 0, multiple = false,
			getOptionSelected, theme
		} = this.properties;
    ...
    const controlProps: any = {
      ...
      classes: this.theme([css.state])
    };
    if (index === activeIndex) { controlProps.checked = true }

		return v('li', {
      key: this._getOptionId(index),
      classes: this.getOptionWrapperClasses()
    }, [
      v('input', controlProps),
      w(ListboxOption, {
	...
	protected render(): DNode {
		const {
      activeIndex = 0, aria = {}, widgetId = this._idBase,
			multiple = false, focus, tabIndex = 0
		} = this.properties;
