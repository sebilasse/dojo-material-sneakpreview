...
*/
	...
	protected _indexResults(query: string = this._value) {
		...
		// TODO : or phonetic ...
		const resultTexts = new Search(results.map(this._boundGetOptionText), {
			shouldSort: sortable,
			threshold: 0.6,
			location: 0,
			distance: 100,
		...
		this._indexed = resultTexts.search(query).map((r, indexedI) => {
	...
	private _onInput(evt: Input | any) {
		...
		//onChange && onChange(this._getValue(evt.value, index), key);
		this.readonlyProp('value', this._value, event);
		this.invalidate()
	}
	...
	protected renderInput() {
		...
		const { blurOnSelect = true, strict = false, results = [] } = this.properties;
		...
		return w(TextInput, {
			...
			autofocus: (!blurOnSelect && this._callInputFocus),
